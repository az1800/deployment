...
function Payment() {
  ...
  const [originalPrice, setOriginalPrice] = useState(parseFloat(amount) || 149.99);
  ...
  const applyDiscount = async () => {
    try {
      ...
      const response = await axios.post('http://localhost:5000/api/coupon/validate', 
        {
          code: discountCode,
          requestId
        },
      ...
      if (response.data.valid) {
        const discountedPrice = originalPrice * (1 - response.data.discountPercentage / 100);
        setPrice(discountedPrice);
        setDiscountApplied(true);
        alert(`Discount of ${response.data.discountPercentage}% applied successfully!`);
      } else {
      ...
      setDiscountApplied(false);
      setPrice(originalPrice);
      alert(error.response?.data?.message || "Failed to validate coupon. Please try again.");
    }
  };
  ...
  return (
    <>
      ...
      <div className="flex flex-col items-center justify-center min-h-screen bg-gray-100 p-4">
        <div className={`w-full max-w-md bg-white rounded-lg shadow-md overflow-hidden transition-all duration-500 ${fadeIn ? "opacity-100" : "opacity-0"}`}>
          <div className="bg-blue-300 p-6">
            <h2 className="text-2xl font-bold text-gray-800 mb-1">Secure Payment via Escrow</h2>
          ...
          <div className="p-6">
            <div className="mb-6 border-b border-gray-200 pb-6">
              <div className="flex justify-between items-center mb-4">
                <h3 className="text-xl font-medium text-gray-700">Premium Financial Services</h3>
                <span className="font-bold text-blue-500 text-lg">${price.toFixed(2)}</span>
              </div>
            ...
            <div className="mb-6">
              <div className="bg-gray-50 p-4 rounded-lg">
                ...
                <p className="text-sm text-gray-600">
                  Your payment will be held safely in escrow until you approve the service delivery.
                  This ensures both parties are protected throughout the transaction.
                </p>
              </div>
            </div>
            ...
            >
              ...
              <input type="hidden" name="non_initiator_email" value="ms115257@gmail.com" />
              <input type="hidden" name="non_initiator_id" value="1360724" />
              <input type="hidden" name="non_initiator_role" value="seller" />
              <input type="hidden" name="title" value={`Accounting Service - Request #${requestId}`} />
              ...
              >
                {paymentStatus === "processing" ? "Processing..." : `Pay $${price.toFixed(2)} Securely`}
              </button>
            </form>
          </div>
        </div>
