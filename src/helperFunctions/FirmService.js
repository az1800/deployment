...
export async function deleteService(serviceId) {
  const url = `http://localhost:5000/api/account_firm/delete-service?id=${serviceId}`;
...
export async function displayServiceData(serviceId) {
  const url = `http://localhost:5000/api/account_firm/getAccountFirmService?id=${serviceId}`;
...
export async function UpdateFirmService(
  ...
  try {
    const response = await axios.post(
      "http://localhost:5000/api/account_firm/update-service",
      uploadData,
